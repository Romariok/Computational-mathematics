...
class Iteration_Method:
    ...
    def solve(self, a, b, estimate):
        ...
        q = max(
            abs(self.equation1.first_derivative(1, 1, 1))
            + abs(self.equation1.first_derivative(1, 1, 2)),
            abs(self.equation2.first_derivative(1, 1, 1))
            + abs(self.equation2.first_derivative(1, 1, 2)),
        )
        self.output += "q = %9.5f\n" % q
        if q > 1:
        ...
        self.output += "x1 = %6.5f x2 = %6.5f f1(x1, x2) = %4.5f f2(x1, x2) = %4.5f Количество итераций: %2d" % (
            int_r(x1),
            int_r(y1),
